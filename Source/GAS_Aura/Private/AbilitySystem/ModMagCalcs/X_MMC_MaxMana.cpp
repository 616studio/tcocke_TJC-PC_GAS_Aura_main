...
UX_MMC_MaxMana::UX_MMC_MaxMana()
{
	...
	TargetIntelligenceDefinition.AttributeToCapture = UX_AttributeSet::GetIntelligenceAttribute();
	TargetIntelligenceDefinition.AttributeSource = EGameplayEffectAttributeCaptureSource::Target;
	TargetIntelligenceDefinition.bSnapshot = false;
	RelevantAttributesToCapture.Add(TargetIntelligenceDefinition);
}

float UX_MMC_MaxMana::CalculateBaseMagnitude_Implementation(const FGameplayEffectSpec& Spec) const
{
	...
	GetCapturedAttributeMagnitude(TargetIntelligenceDefinition, Spec, EvaluationParameters, Intelligence);
	
	// Clamp the value to ensure the Attribute is never a negative number, which would break the math (at least for this class's example).
	...
	return BaseValue + (IntelligenceMultiplier * Intelligence) + (LevelMultiplier * CharacterLevel);
}